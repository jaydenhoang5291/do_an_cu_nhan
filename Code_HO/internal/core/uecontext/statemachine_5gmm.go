...
func mm_RegisteredInitiated(state *fsm.State, event *fsm.EventData) {
	...
	case model.RegistrationAcceptEvent:
		...
		ueCtx.timerEngine.RemoveTimer(timer.T3502, int64(ueCtx.id))
		ueCtx.timerEngine.RemoveTimer(timer.T3510, int64(ueCtx.id))
		ueCtx.timerEngine.RemoveTimer(timer.T3511, int64(ueCtx.id))
		if ueCtx.enableFuzz {
	...
	case model.RegistrationRejectEvent, model.AuthFailEvent, model.SecurityModeFailEvent, model.MissingInfo:
		stats.GlobalStats.FailProcedure(stats.ProcRegistration)
		if exist, _, err := ueCtx.timerEngine.GetTimerStatus(timer.T3510, int64(ueCtx.id)); exist && err == nil {
			ueCtx.timerEngine.StopWithExpireFunc(timer.T3510, int64(ueCtx.id))
		} else if exist, _, err = ueCtx.timerEngine.GetTimerStatus(timer.T3511, int64(ueCtx.id)); exist && err == nil {
			ueCtx.timerEngine.TriggerTimeout(timer.T3511, int64(ueCtx.id))
		}
