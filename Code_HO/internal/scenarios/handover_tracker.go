...
func (ht *HandoverTracker) CompleteHandoverWithTiming(step int, success bool, failureReason string, endTime time.Time, duration time.Duration) {
	ht.mu.Lock()
	defer ht.mu.Unlock()

	for i := len(ht.Results) - 1; i >= 0; i-- {
		if ht.Results[i].Step == step && ht.Results[i].EndTime.IsZero() {
			ht.Results[i].EndTime = endTime
			ht.Results[i].Duration = duration
			ht.Results[i].Success = success
			ht.Results[i].FailureReason = failureReason
