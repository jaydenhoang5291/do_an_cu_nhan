...
func SendToGnb(source string, msg rlink.Message, targetGnbId string, isInnerGnb bool) error {
	...
	// Xn simulation: packet loss
	if isInnerGnb {
		if gnb.controlPlaneInfo.xnPacketLoss > 0 && rand.Float64() < gnb.controlPlaneInfo.xnPacketLoss {
			fmt.Fprintf(os.Stderr, "DEBUG Xn DROP: loss=%.2f msg=%s target=%s\n",
				gnb.controlPlaneInfo.xnPacketLoss, msg.GetType(), targetGnbId)
			return fmt.Errorf("Xn packet lost: %s to %s", msg.GetType(), targetGnbId)
		}

		if gnb.controlPlaneInfo.xnLatencyMs > 0 {
			fmt.Fprintf(os.Stderr, "DEBUG Xn DELAY: %dms msg=%s target=%s\n",
				gnb.controlPlaneInfo.xnLatencyMs, msg.GetType(), targetGnbId)
		}

		// Xn simulation: delay + jitter
		xnDelay := time.Duration(gnb.controlPlaneInfo.xnLatencyMs) * time.Millisecond
		if gnb.controlPlaneInfo.xnJitterMs > 0 {
			j := time.Duration(rand.Int63n(int64(gnb.controlPlaneInfo.xnJitterMs)*int64(time.Millisecond))) -
				time.Duration(gnb.controlPlaneInfo.xnJitterMs/2)*time.Millisecond
			xnDelay += j
			if xnDelay < 0 {
				xnDelay = 0
			}
...
// sendCriticalMsgToUe bypasses radio loss/jitter simulation.
// Used for critical control messages that must reach the UE
// (handover commands, CHO configs, RRC signaling).
func (ue *GnbUeContext) sendCriticalMsgToUe(msg rlink.Message) {
	ue.rlinkConn.SendDownlinkCritical(msg)
}
