...
class UMaAVLosProbabilityTests(unittest.TestCase):
    def test_uma_height_1_5_to_22_5_uses_38901_formula(self):
        h_ut = 20.0
        d2d = 100.0
        ...
        actual = RadioModel.uma_av_los_probability(d2d, h_ut)

        self.assertAlmostEqual(actual, expected)

    def test_uma_height_at_1_5_is_valid(self):
        self.assertEqual(RadioModel.uma_av_los_probability(10.0, 1.5), 1.0)

    def test_height_above_100_has_full_los_probability(self):
        self.assertEqual(RadioModel.uma_av_los_probability(1000.0, 200.0), 1.0)
        self.assertEqual(RadioModel.uma_av_los_probability(1000.0, 300.0), 1.0)

    def test_distance_below_d1_has_full_los_probability(self):
        self.assertEqual(RadioModel.uma_av_los_probability(10.0, 50.0), 1.0)
    ...
    def test_invalid_height_raises_error(self):
        with self.assertRaises(ValueError):
            RadioModel.uma_av_los_probability(100.0, 1.4)

        with self.assertRaises(ValueError):
            RadioModel.uma_av_los_probability(100.0, 301.0)
