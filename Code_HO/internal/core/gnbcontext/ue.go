...
type GnbUeContext struct {
	*logger.Logger
	...
	handoverSourceGnbId string          // Source gNB for incoming Xn handover
...
func (ue *GnbUeContext) SetHoState(state model.StateType) {
	ue.lock.Lock()
	defer ue.lock.Unlock()
	ue.stateHo = state
}

func (ue *GnbUeContext) GetHoState() model.StateType {
	ue.lock.Lock()
	defer ue.lock.Unlock()
	return ue.stateHo
}
