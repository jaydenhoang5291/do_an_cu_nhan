...
type UeContext struct {
	*logger.BufferedLogger
	...
	stateHo model.StateType // HO state

	// CHO (Conditional Handover) fields
	choCandidates     []model.CandidateGnb
	choConditions     []model.ChoCondition
	choEnabled        bool
	choTriggerTimer   *time.Ticker
	rsrpValues        map[string]float64
	rsrpMu            sync.RWMutex
	measurementTicker *time.Ticker
...
func CreateUe(
	...
	ue.timerEngine = timer.NewTimerEngine(ue.ctx)
...
func (ue *UeContext) GetSMState() *fsm.State {
	if ue.sessions[1] != nil {
		return ue.sessions[1].state_sm
	}
	return nil
}
...
func (ue *UeContext) GetRlinkStats() rlink.Stats {
	ue.mutex.Lock()
	conn := ue.rlinkConn
	ue.mutex.Unlock()
	if conn == nil {
...
func (ue *UeContext) GetTimerEngine() *timer.TimerEngine {
	return ue.timerEngine
}
