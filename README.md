...
- `config.py`: Contains global configuration variables such as radio parameters (transmit power `PTX`, gain), UE height range, height-dependent UE speed ranges, shadow-fading update distance, UE stop-and-go duration settings, and noise power.
...
- **Configurable UE Height**: Allows users to enter any UE height in the supported 1.5--300 m range. The radio model selects the appropriate UMa or UMa-AV calculation branch from that height.
- **Height-Dependent UE Speed**: Randomizes UE speed from a configured range selected by the entered UE height.
