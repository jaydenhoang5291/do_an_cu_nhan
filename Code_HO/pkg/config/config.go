...
type Config struct {
	...
	Simulation   SimulationConf `yaml:"simulation"`
}

type SimulationConf struct {
	Enable       bool    `yaml:"enable"`
	PacketLoss   float64 `yaml:"packetLoss"`   // 0.0-1.0
	LatencyMs    int     `yaml:"latencyMs"`    // milliseconds
	JitterMs     int     `yaml:"jitterMs"`     // milliseconds
	XnPacketLoss float64 `yaml:"xnPacketLoss"` // Xn path packet loss 0.0-1.0
	XnLatencyMs  int     `yaml:"xnLatencyMs"`  // Xn path one-way delay ms
	XnJitterMs   int     `yaml:"xnJitterMs"`   // Xn path jitter ms
}
