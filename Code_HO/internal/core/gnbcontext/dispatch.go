...
func (gnb *GnbContext) dispatch(amf *GnbAmfContext, ngapPdu []byte) {
	...
	// handle NGAP message.
	gnb.Trace("Dispatching NGAP message: Present=%d", ngapMsg.Present)
	...
	case ies.NgapPduUnsuccessfulOutcome:
		...
		case ies.ProcedureCode_PathSwitchRequest:
			gnb.LogNgapReceive("PathSwitchRequestFailure")
			//taskLog.Task = "Task: handle Receive PathSwitchRequestFailure msg"
			innerMsg := ngapMsg.Message.Msg.(*ies.PathSwitchRequestFailure)
			gnb.handlerPathSwitchRequestFailure(innerMsg)
