...
func (ue *UeContext) handleConfigurationUpdateCommand(message *nas.ConfigurationUpdateCommand) {
	...
	// Start/Restart T430: NTN satellite info validity timer
	ue.timerEngine.CreateTimer(timer.TimerConfig{
		TimerType: timer.T430,
		Duration:  timer.T430_duration,
		CountMax:  1,
		ExpireFunc: func() {
			ue.Warn("T430 expired: satellite position data invalid, mark invalid")
		},
	}, int64(ue.id))
	ue.timerEngine.Start(timer.T430, int64(ue.id))
