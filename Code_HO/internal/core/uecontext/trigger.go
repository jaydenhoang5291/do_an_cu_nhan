...
func (ue *UeContext) triggerSwitchToIdle() {
	...
	// Start T310: Radio link monitoring timer
	ue.timerEngine.CreateTimer(timer.TimerConfig{
		TimerType: timer.T310,
		Duration:  timer.T310_duration,
		CountMax:  1,
		ExpireFunc: func() {
			ue.Warn("T310 expired: radio link failure detected")
			// Stop T312 if running (measurement reporting no longer needed)
			ue.timerEngine.Stop(timer.T312, int64(ue.id))
			// Start T311: cell selection timer
			ue.timerEngine.CreateTimer(timer.TimerConfig{
	...
	ue.timerEngine.Start(timer.T310, int64(ue.id))

	// Start T312: Measurement reporting timer during radio link problems
	ue.timerEngine.CreateTimer(timer.TimerConfig{
		TimerType: timer.T312,
		Duration:  timer.T312_duration,
		CountMax:  1,
		ExpireFunc: func() {
			ue.Info("T312 expired: measurement report triggered during radio link monitoring")
		},
	}, int64(ue.id))
	ue.timerEngine.Start(timer.T312, int64(ue.id))
