...
The implementation uses the same UMa/UMa-AV wrapper functions for all UE heights. When \(h_{\mathrm{UE}}=1.5\) m, these functions reduce to the low-height UMa case. The only ground-default difference in the shadow-fading implementation is that, when no elevated UE height is enabled, the standard deviation is taken from the fixed values in \texttt{config.py}: \(\sigma_{\mathrm{SF}}=4.6\) dB for LOS and \(10\) dB for NLOS.
...
HO is the mobility procedure that transfers a UE from a source cell or gNB to a target cell or gNB while attempting to maintain service continuity. From a radio perspective, an HO may be motivated by changes in serving-cell and neighbor-cell quality. From a protocol perspective, the procedure requires signaling between the UE, source gNB, target gNB, and core network before the transition can be considered complete.

In this thesis, HO delay refers to the time interval required for the HO procedure to progress from its trigger or preparation phase to its observed completion or failure outcome in the available experiment logs. This delay is affected not only by radio conditions, but also by signaling latency, packet loss, jitter, retransmission behavior, and timer configuration. Therefore, HO-delay analysis requires protocol-level event logs in addition to radio-measurement traces.

\subsection{Open-source platforms of 5G core mobile network}

Open-source 5G core platforms provide practical software implementations of the core network functions used in 5G Standalone (SA) systems. They are useful for research and experimental testbeds because they allow the control-plane and user-plane behavior of a mobile core network to be deployed, configured, and observed without relying on a commercial 5GC product. In this thesis, two representative open-source platforms are considered: free5GC and Open5GS.

free5GC is an open-source 5GC implementation whose goal is to implement the 5GC defined in 3GPP Release 15 and beyond. The project describes itself as a Linux Foundation project for 5G mobile core networks, with support for SBA and cloud-native deployment using Docker or Kubernetes \cite{free5gc_project}. Its modular implementation of network functions such as AMF, SMF, UPF, NRF, UDM, AUSF, PCF, and NSSF makes it suitable for experiments that need a 5G SA core with explicit control-plane signalling.

Open5GS is another open-source mobile core platform. It is designed for building NR/LTE mobile networks and provides both 5GC and EPC support, which makes it useful when experiments need to compare or reuse components across 5G and LTE-oriented deployments \cite{open5gs_project}. Compared with free5GC, Open5GS has a broader multi-generation scope, while free5GC focuses more directly on a 5GC implementation aligned with the 5G core architecture.

\begin{table}[H]
\centering
\caption{Comparison of free5GC and Open5GS}
...
\hline
Suitability for this thesis & Suitable when the experiment requires a 5G SA core and explicit 5GC signalling behavior. & Suitable as a comparative platform or when LTE/EPC compatibility is also required. \\
\hline
\end{tabularx}
\end{table}

In the context of this thesis, free5GC is the more relevant platform because the experimental system focuses on 5G core signalling and handover-related behavior in a 5G-oriented environment. Open5GS is still important as a reference platform because it represents a widely used open-source alternative for constructing NR/LTE mobile core testbeds.
