...
This project is a Python-based cellular network simulator that visualizes and logs the behavior of User Equipments (UEs) moving within a Manhattan grid coverage area. It models base station (BS) connectivity, radio signal propagation, path loss, shadow fading, and handing over between cells, with an option to include a Flying Base Station (UAV-BS).

## Project Structure

- `main.py`: The entry point of the application. Prompts the user for simulation parameters (area size, number of UEs, grid spacing, UAV settings) and launches the simulation.
...
- **UAV Base Station Option**: Allows users to dynamically inject a Line-of-Sight (LOS) UAV-BS acting as an aerial macro-cell with distinct propagation properties.
...
4. Add UAV-BS coverage (y/N) - if yes, prompts for radius, altitude, and transmit power.
