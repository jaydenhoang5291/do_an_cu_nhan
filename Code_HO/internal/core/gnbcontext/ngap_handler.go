...
func (gnb *GnbContext) handlerUeContextReleaseCommand(msg *ies.UEContextReleaseCommand) {
	...
	// Stop TNGRELOCoverall: N2 handover overall execution complete
	gnb.timerEngine.Stop(timer.TNGRELOCoverall, ue.prUeId)
	gnb.timerEngine.Stop(timer.TNGRELOCprep, ue.prUeId)
...
func (gnb *GnbContext) handlerPathSwitchRequestAcknowledge(msg *ies.PathSwitchRequestAcknowledge) {
	...
	// Xn relocation timers run on the source gNB; stop them there once path
	// switch completes on the target gNB.
	stopXnRelocTimersOnSource(ue.handoverSourceGnbId, ue.prUeId)
	ue.Info("TXnRELOC timers stopped on source gNB %s for UE %d", ue.handoverSourceGnbId, ue.prUeId)
...
func (gnb *GnbContext) handlerPathSwitchRequestFailure(msg *ies.PathSwitchRequestFailure) {
	causeStr := "unknown"
	// switch msg.Cause.Choice {
	// case ies.CausePresentRadionetwork:
	// 	if msg.Cause.RadioNetwork != nil {
	// 		causeStr = fmt.Sprintf("RadioNetwork(%d)", msg.Cause.RadioNetwork.Value)
	// 	}
	// case ies.CausePresentTransport:
	// 	if msg.Cause.Transport != nil {
	// 		causeStr = fmt.Sprintf("Transport(%d)", msg.Cause.Transport.Value)
	// 	}
	// case ies.CausePresentNas:
	// 	if msg.Cause.Nas != nil {
	// 		causeStr = fmt.Sprintf("NAS(%d)", msg.Cause.Nas.Value)
	// 	}
	// case ies.CausePresentProtocol:
	// 	if msg.Cause.Protocol != nil {
	// 		causeStr = fmt.Sprintf("Protocol(%d)", msg.Cause.Protocol.Value)
	// 	}
	// case ies.CausePresentMisc:
	// 	if msg.Cause.Misc != nil {
	// 		causeStr = fmt.Sprintf("Misc(%d)", msg.Cause.Misc.Value)
	// 	}
	// }
	gnb.Error("PathSwitchRequestFailure received from AMF (AMF-UE-NGAP-ID: %d, RAN-UE-NGAP-ID: %d, Cause: %s)",
		msg.AMFUENGAPID, msg.RANUENGAPID, causeStr)

	ue := gnb.getUeFromContext(msg.RANUENGAPID, msg.AMFUENGAPID)
	if ue == nil {
		gnb.Error("Cannot handle PathSwitchRequestFailure for unknown UE With RANUEID %d", msg.RANUENGAPID)
		return
	}

	// Stop TXnRELOC timers on the source gNB for Xn handover failures.
	stopXnRelocTimersOnSource(ue.handoverSourceGnbId, ue.prUeId)
	gnb.Info("TXnRELOC timers stopped (cancel) on source gNB %s for UE %d", ue.handoverSourceGnbId, ue.prUeId)

	gnb.Error("Path Switch procedure failed for UE-ranNgapId-%d", ue.ranUeNgapId)
}
...
func (gnb *GnbContext) handlerHandoverCommand(amf *GnbAmfContext, msg *ies.HandoverCommand) {
	...
	// Stop TNGRELOCprep: N2 handover preparation complete
	gnb.timerEngine.Stop(timer.TNGRELOCprep, ue.prUeId)

	if HandoverStateCallback != nil {
		HandoverStateCallback(ue.prUeId, model.HO_ExecuteEvent)
	}
}
